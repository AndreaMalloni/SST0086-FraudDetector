...
class FidelityAnalyzer:
    ...
    def analyze_fidelity(
        ...
        """
        results = {"aufc_scores": [], "correlations": [], "plots": []}
        ...
        for idx, instance in instances.iterrows():
            ...
            plt.title(f"Fidelity Analysis - Instance {idx}")
            plt.grid(True)
            plt.legend()
            ...
            # Save the plot
            plot_path = (
                plots_dir / f"fidelity_{idx}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
            )
    ...
    def _plot_fidelity_curve(
        self, instance: pd.Series, feature_importance: dict[str, float], idx: int
    ):
